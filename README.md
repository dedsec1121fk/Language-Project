...
The repository also contains `.gitattributes` and a language-balance audit targeting **at least 0.2% for every tracked GitHub Linguist language group**. The balance comes from useful module source/recipes, not from hiding Python. GitHub Linguist remains the final authority after push.
...
Το repository στοχεύει **τουλάχιστον 0,2% για κάθε GitHub Linguist language group που μπορεί πραγματικά να εμφανίσει το GitHub**. Οι γλώσσες που δεν αναγνωρίζονται ξεχωριστά από το Linguist παραμένουν στο project, αλλά αποκλείονται από λανθασμένη ταξινόμηση μέσω `.gitattributes`.
