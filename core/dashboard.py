...
def dashboard():
    active=active_languages('registry');cat=catalog_stats();cal=load_calibration();db=db_stats();cps=checkpoint_list();incomplete=[x for x in cps if x.get('status')!='complete'];sc=load_scenarios().get('scenarios',{})
    ...
    print('='*96)
    return {'active':len(active),'catalog':cat,'calibration':cal,'database':db,'incomplete_checkpoints':incomplete,'scenarios':list(sc)}
