...
def snapshot():
    state=ROOT/'state'/'active.json';cal=ROOT/'state'/'calibration.json'
    try:s=json.loads(state.read_text())
    except Exception:s={}
    return {
        'registry_sha256':_sha(ROOT/'languages.json'),'manifest_sha256':_sha(ROOT/'MANIFEST.json'),
        ...
        'prefix':os.environ.get('PREFIX',''),'termux_version':os.environ.get('TERMUX_VERSION','')
    }
