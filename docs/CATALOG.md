...
Catalog-only entries never enter the benchmark chain. Executability is controlled exclusively by `languages.json` plus the on-device compile/start/PING/round-trip tests in `scripts/setup.py`.
